...
var importResponse = async (response, path, collection = {}, type = "buffer") => {
  ...
  if (mimeType === 'text/typescript') {
      if (!window.ts) await loadScriptTag('https://cdn.jsdelivr.net/npm/typescript/lib/typescriptServices.js')
      
      const tsCode = (type !== 'buffer') ? info : new TextDecoder().decode(info);

      if (!tsCode) console.warn('TS Code not found!', tsCode, info, type)
      response.text = window.ts.transpile(tsCode, tsconfig.compilerOptions);
      info = response[type] = (type !== 'buffer') ? response.text :  new TextEncoder().encode(response.text); // encode to buffer
...
const getPath2 = (str, path, base) => get(str, base, false, path.split("/").length === 1);

const getPackagePath = (path, base=path) => {
  const res = getPath2("package.json", path, base);
  return res
}
...
const getMainPath = async (path, base=path) => {
  ...
  const res = getPath2(destination, path, base);
  return res
}
...
const handleNoExtension = (path, repExt='js') => {
  ...
  if (!isAbsolute || (isAbsolute && split.length > 1)) {
      if (!mimeTypeMap[ext]) return`${path}.${repExt}` // use ts internally
  }
  ...
  if (typeof handler === 'string' && (!force || force === 'string')) {
    return handleNoExtension(path, handler)
     ...
     return await handler(path).catch(e => {
      console.log('Failed at the end', e)
...
const internalImport = async (info, opts, handler = 'js') => {
  ...
  const isAbsolute = path[0] !== ".";
      ...
      try {
        ...
        let correctPath = get(path, info.uri); 
      ...
      if (!ref) {
        ...
        // if (url != correctPath) console.warn('Different!', url, correctPath)
        const willBeJS = extension2.includes("js") || extension2 === 'ts';
        const newURI = dependentFileWithoutRoot;
          ...
          if (filesystemFallback)
            ...
            await importResponse({text: importedText}, correctPath, uriCollection, "text").catch(e => {
              console.log('res reror', e)
            });
        }
...
var safeImport = async (uri, opts = {}) => {
  ...
  if (!module || outputText) {
    ...
    if (noExtension) {
      ...
      do {
        const transformed = await handleHandlers(uri, toTry.pop()).catch(e => {
          throw e
        })
    ...
    if (!response.buffer) throw new Error(`Response not found for ${uri}!`)
    ...
    try {
      ...
      module = await importResponse(response, uri, uriCollection, "text").catch(e => { throw e })
    }
  }
